...
class Mago(Personagem):
    def __init__(self, nome):
        super().__init__(nome, hp=70, ataque=15, defesa=5, critico=0.2, esquiva=0.1, mana=50)
        self.classe = "Mago"
    ...
    def atacar(self, alvo):
        ...
        if random.random() < self.critico:
            print("**CRITICO**")
            dano *= 2
        print(f"{self.nome} ataca {alvo.nome} com um bastão mágico causando {dano} de dano.")
...
class Berserker(Personagem):
    def __init__(self, nome):
        super().__init__(nome, hp=90, ataque=12, defesa=10, critico=0.1, esquiva=0.07, mana=20)
    ...
    def atacar(self, alvo):
        ...
        if random.random() < self.critico:
            print("**CRITICO**")
            dano *= 2
        print(f"{self.nome} desfere um golpe brutal em {alvo.nome}, causando {dano:.0f} de dano.")
        alvo.receber_dano(dano)
    ...
    def curar(self):
        cura = 15
        self.hp += cura
        print(f"{self.nome} grita de fúria e regenera {cura} de HP.")

class Arqueiro(Personagem):
    def __init__(self, nome):
        super().__init__(nome, hp=80, ataque=10, defesa=6, critico=0.3, esquiva=0.3, mana=20)
    ...
    def atacar(self, alvo):
        ...
        if crit:
            print("**CRITICO**")
            dano *= 2
        print(f"{self.nome} dispara uma flecha em {alvo.nome}, causando {dano:.0f} de dano.")
        alvo.receber_dano(dano)
    ...
    def curar(self):
        cura = 10
        self.hp += cura
        print(f"{self.nome} aplica uma bandagem e recupera {cura} de HP.")
